...
(defun zk-zorg--filter-tag-match-string (tag-match-string)
  (if (string-match-p "+\\|^tbdsc\\b" tag-match-string)
      ;; if "tbdsc" is explicitly added, leave it as-is
      tag-match-string
    ;; otherwise, exclude "tbdsc"
    (concat "-tbdsc+" tag-match-string)))
...
the match string. "
  ...
  (let ((tags (read-string "View for tags: " nil 'org-tags-history nil t)))
    ...
    (org-tags-view arg (zk-zorg--filter-tag-match-string tags))))
...
(defun zk-zorg-rsync-download ()
 ...
 according to the result."
  ...
  (setq zk-zorg-retr-destid-to-src-entry-mp-up-to-date-p nil)
...
(defun zk-zorg-retr--create-entry-alist-for-current-entry ()
  ...
  (save-excursion
    ...
    (let* ((element (or (org-element-at-point)
           ...
           (heading-link (zk-zorg-retr--get-current-heading-link))
...
(defconst zk-zorg-retr-link-regex
  "\\[\\[file:[^][:#]*::#\\([^][:#]*\\)\\]\\[\\([^][:#]*\\)\\]"
...
(defconst zk-zorg-retr-buffer-name-prefix "*zorg/retr* ")

(defun zk-zorg-retrs-for-tags--find-root-entries (tag-match)
  "Scan the whole repo and create a list that contains the alists of
entries that match the given tag-match string.  Those are considered root
entries.  The resulting list is set to the buffer-local variable
`zk-zorg-retr-root-entry-alists', sorted by date."
...
(defun zk-zorg-retr--refresh-index ()
  "Call `zk-zorg-retr--create-index' when necessary."
  (unless zk-zorg-retr-destid-to-src-entry-mp-up-to-date-p
    (zk-zorg-retr--create-index)))

(defun zk-zorg-retr--entry-sort-comparator (e1 e2)
  (string> (alist-get ':date e1)
           (alist-get ':date e2)))

(defun zk-zorg-retr--create-index ()
             ...
             (while (re-search-forward link-regex (region-end) t)
               (let ((dest-id (match-string-no-properties 1))
                 ...
                 (unless (string-match-p
                          ;; Exclude timestamped references generated by
                          ;; "C-c l w" which links to a recent update
                          ;; for the current heading.  Because that
                          ;; update entry already has a back reference
                          ;; to the current heading, adding this forward
                          ;; this reference is duplicate.
                          "^[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9][^)]*$"
                          link-text)
...
(defun zk-zorg-retr--print-entry
  ...
  (let* ((level (length ancestor-links))
         ...
         (src-entry-alists
          (zk-multimap-get zk-zorg-retr-destid-to-src-entry-mp
                           custom-id))
...
(defvar-local zk-zorg-retr-refresh-form nil
  "The form to be evaluated to refresh the ref tree buffer")

(defvar zk-zorg-retr-destid-to-src-entry-mp-up-to-date-p nil
  "t if `zk-zorg-retr-destid-to-src-entry-mp' is still
...
(defun zk-zorg-retr--open-link (&optional other-window)
  (interactive)
  (let* ((pos-alist (zk-zorg-retr--get-heading-pos-at-point))
...
(defun zk-zorg-retr--open-link-other-window ()
  (interactive)
  (zk-zorg-retr--open-link t))
...
(defun zk-zorg-retr-command (&optional arg)
...
Respects `org-agenda-sticky'.  If it's `t' and there is a preexisting
ref tree buffer for the same entry, will switch to this buffer without
refreshing it.

Headings tagged with any tag from `zk-zorg-retr-tag-exclude-list'
...
(defun zk-zorg-retrs-for-tags (tag-match-string)
  "Implementation for `zk-zorg-retrs-for-tags-command'."
  (let* ((tag-match (zk-zorg--filter-tag-match-string tag-match-string))
         (output-buffer-name (concat zk-zorg-retr-buffer-name-prefix "m:" tag-match))
         (output-buffer (get-buffer output-buffer-name)))
    (if (and output-buffer org-agenda-sticky)
        (message "Sticky reftree buffer, use ‘g’ to refresh")
      (setq output-buffer (zk-recreate-buffer output-buffer-name
...
(defun zk-zorg-retrs-for-tags-command ()
  "Create a buffer to display the reference trees of all entries that
match the given tag-match string.

Respects `org-agenda-sticky'.  If it's `t' and there is a preexisting
ref tree buffer with the same match string, will switch to this buffer
without refreshing it."
  (interactive)
  (let ((tags (read-string "Reference trees for tags: " nil 'org-tags-history nil t)))
    (unless (> (length tags) 0)
      (user-error "No tag entered."))
    (zk-zorg-retrs-for-tags tags)))

(defun zk-zorg-retr-previous-buffer ()
  "Switch to the most recently visited back reference buffer."
  (interactive)
  (zk-previous-buffer-regex
   (concat "^" (regexp-quote zk-zorg-retr-buffer-name-prefix))))

(defun zk-zorg-retr--expand-at-point ()
        ...
        (progn
          (let* ((entry-alist
                 ...
                 (entry-ancestor-links
                  (get-text-property (point) 'zk-zorg-retr-entry-ancestor-links))
                 (level (length entry-ancestor-links))
                 (inhibit-read-only t)
                 (entry-expansion-state
                  (get-text-property (point) 'zk-zorg-retr-entry-expansion-state)))
            (cl-assert entry-alist t)
            (cond
