...
`zk-zorg-retr-root-entry-alists'."
...
(defun zk-zorg-retrs-for-tags (tag-match)
  "Implementation for `zk-zorg-retrs-for-tags-command'."
  (let* ((output-buffer-name (concat zk-zorg-retr-buffer-name-prefix "m:" tag-match))
         (output-buffer (get-buffer output-buffer-name)))
    (if (and output-buffer org-agenda-sticky)
        (message "Sticky reftree buffer, use ‘g’ to refresh")
      (setq output-buffer (zk-recreate-buffer output-buffer-name))
          ...
          (dolist (entry-alist zk-zorg-retr-root-entry-alists)
            (when
                ...
                (let ((custom-id (alist-get ':custom-id entry-alist)))
                  (and custom-id
                       (zk-multimap-get
                        zk-zorg-retr-destid-to-src-entry-mp
                        custom-id)))
              (zk-zorg-retr--print-entry
               nil entry-alist nil))
            (cl-incf counter)
