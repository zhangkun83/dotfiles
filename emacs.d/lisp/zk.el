...
(defun zk-previous-buffer-regex (regex)
  "Switch to the most recently visited buffer whose name matches the REGEX."
  (let ((candidate-buffers
    ...
    (unless candidate-buffers
      (user-error "No previous buffer to switch to"))
    (let ((current-buffer (current-buffer))
          (buffer (car candidate-buffers)))
  ...
  (let* ((lock-dir (concat zk-user-home-dir "/.zk-emacs-server-locks/"))
    ...
    (if (not servers)
      ...
      (when (and (boundp 'server-process) 
        ...
        (setq servers (delete server-name servers)))
		  ...
		  (= monitor-height-pixels 2400))
            ...
            ((and (= monitor-height-pixels 1080)
             ...
             (progn
               ...
               (setq frame-height-pixels 750)))
            ((and (= monitor-height-pixels 3084)
                  (= monitor-dpi 199))
             (progn
